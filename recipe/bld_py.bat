...
set "CMAKE_ARGS=%CMAKE_ARGS% -G Ninja -DMOMENTUM_BUILD_IO_USD=OFF -DMOMENTUM_BUILD_RENDERER=ON -DMOMENTUM_BUILD_TESTING=OFF -DMOMENTUM_ENABLE_SIMD=OFF -DMOMENTUM_USE_SYSTEM_GOOGLETEST=ON -DMOMENTUM_USE_SYSTEM_PYBIND11=OFF -DMOMENTUM_USE_SYSTEM_RERUN_CPP_SDK=ON -DCMAKE_POLICY_DEFAULT_CMP0148=NEW -DPYBIND11_PYTHON_VERSION=%PYTHON_VER% -DPython3_ROOT_DIR=%PYTHON_PREFIX_CMAKE% -DPython3_EXECUTABLE=%PYTHON_CMAKE% -DPython3_LIBRARY=%PYTHON_LIB_CMAKE% -DPython3_INCLUDE_DIR=%PYTHON_INCLUDE_CMAKE% -DPython3_FIND_STRATEGY=LOCATION -DPython3_FIND_REGISTRY=NEVER"
